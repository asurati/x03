...
int bitmap_on_off(struct bitmap *map, int start, int num_bits, char is_on)
{
	...
	if (map == NULL || start < 0 || num_bits <= 0) return ERR_PARAM;
	if (start + num_bits <= 0) return ERR_PARAM;
	if (start + num_bits > map->num_bits) return ERR_PARAM;
		     ...
		     char is_on)
{
	...
	for (i = 0; i < num_bits; ++i) {
		...
		if (is_on && val == 0) return ERR_UNEXP;
		if (!is_on && val) return ERR_UNEXP;
...
static
int bitmap_find_on_off(const struct bitmap *map, int align, int num_bits,
		       char is_on)
{
	uint64_t val;
	int i, ix, bit, decr, start;

	if (map == NULL || num_bits <= 0 || num_bits > map->num_bits)
		return ERR_PARAM;

	decr = 1 << align;
	start = align_down(map->num_bits - 1, decr);

	for (; start >= 0; start -= decr) {
		ix = start >> 5;
		bit = start & 0x1f;
		for (i = 0; i < num_bits && start + i < map->num_bits; ++i) {
			val = map->buf[ix] & (1ul << bit);

			if (is_on && val == 0) break;
			if (!is_on && val) break;
	...
	if (start < 0) return ERR_NOT_FOUND;
	return start;
}
...
int bitmap_find_off(const struct bitmap *map, int align, int num_bits)
{
	return bitmap_find_on_off(map, align, num_bits, 0);
}
